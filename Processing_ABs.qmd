...
Folder <- file.path("/media", "david", "DavidRach", "SingleColors", "AlphaBetaPrep")
...
Dates2 <- FileSort(list=Stained, study=Study, type=Type, controlType=controlType)
#Dates2 <- str_extract(Stained, paste0("(?<=", Study, "/)[^/]+(?=/", Antigen, ")"))


if (length(Dates1) != length(Dates2)){
    message("Different number of unstained to single color controls")

    if (length(Dates1) > length(Dates2)){
        Extra <- setdiff(Dates1, Dates2)
        Dates1 <- Dates1[!Dates1 %in% Extra]
    } else {Extra <- setdiff(Dates2, Dates1)
        Dates2 <- Dates2[!Dates2 %in% Extra]
    }
}
...
```{r}
Data <- Luciernaga_SignatureExternalUnstained(x=Dates1[1:21], UnstainedList=Unstained,
