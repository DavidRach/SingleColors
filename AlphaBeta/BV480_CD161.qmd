...
TheseDates <- Data1 |> arrange(Experiment) |> pull(Experiment) |> unique()
...
StartDate <- head(TheseDates, 1)
EndDate <- tail(TheseDates, 1)

MFI <- MFI |> filter(DATE >= StartDate & DATE <= EndDate)
